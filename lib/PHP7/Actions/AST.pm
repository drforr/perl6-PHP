...
=head1 PHP7::Actions::AST

C<PHP7::Actions::AST> encompasses the grammar actions needed to create a
perl6 AST from an PHP7 parser specification.

=head1 Synopsis

    use PHP7::Actions::AST;
    use PHP7::Grammar;
    my $a = PHP7::Actions::AST.new;
    my $g = PHP7::Grammar.new;
...
tree (AST) of the PHP7 grammar perl6 has been asked to parse. Other variants
...
the PHP7 grammar, minus the comments and unimportant syntax. Where order is
...
most complex, and is described in detail at the appropriate place.
  ...
  monolithic block of Java code. If you're converting a grammar from PHP7 to
  Perl6 you'll need to take note of this behavior, but it's currently beyond
  the scope of this action to parse the text here.

  =item content

  To preserve ordering in case we want to round-trip PHP7-Perl6-PHP7, this
  ...
  The real fun begins inside the C<content> key. Even a simple PHP7 rule
  ...
  The PHP7 grammar assumes that rules always start with a list of
  ...
  PHP7 has two general kinds of groups - Implicit groups and explicit.
  ...
  those that override PHP7's assumptions, such as C<number : sign? (a b)?>.
