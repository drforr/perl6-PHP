...
=head1 PHP5::Actions::AST

C<PHP5::Actions::AST> encompasses the grammar actions needed to create a
perl6 AST from an PHP5 parser specification.

=head1 Synopsis

    use PHP5::Actions::AST;
    use PHP5::Grammar;
    my $a = PHP5::Actions::AST.new;
...
tree (AST) of the PHP5 grammar perl6 has been asked to parse. Other variants
...
the PHP5 grammar, minus the comments and unimportant syntax. Where order is
...
most complex, and is described in detail at the appropriate place.
  ...
  monolithic block of Java code. If you're converting a grammar from PHP5 to
  Perl6 you'll need to take note of this behavior, but it's currently beyond
  the scope of this action to parse the text here.

  =item content

  To preserve ordering in case we want to round-trip PHP5-Perl6-PHP5, this
  ...
  The real fun begins inside the C<content> key. Even a simple PHP5 rule
  ...
  The PHP5 grammar assumes that rules always start with a list of
  ...
  PHP5 has two general kinds of groups - Implicit groups and explicit.
  ...
  those that override PHP5's assumptions, such as C<number : sign? (a b)?>.
